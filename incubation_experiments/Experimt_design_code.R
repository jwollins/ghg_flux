...
# 2 cover crops
TrtCC   <- paste0("CC", 1:2)    # P1 & P2
n_TrtCC <- n_distinct(TrtCC) # 2

# 2 soil treatments - WFPS 60/90%
TrtSoil   <- paste0("S", 1:2)    # S1 - S4
n_TrtSoil <- n_distinct(TrtSoil) # 4

# 2 chop lenghts
TrtChop   <- paste0("N", 1:2)    # N1 - N6
n_TrtChop <- n_distinct(TrtChop) # 6
...
fac2rcbd_out <- design.ab(trt = c(n_TrtCC, n_TrtSoil), 
                          design = "rcbd",
                          r = n_Reps, 
                          seed = 42)
...
# Add Row and Col 
fac2rcbd_out$bookRowCol <- fac2rcbd_out$book %>%
  bind_cols(expand.grid(Row = 1:n_TrtCC,
                        Col = 1:(n_TrtSoil*n_Reps))) %>% 
  mutate(TrtCC = paste0("N", A),
         TrtSoil = paste0("S", B))
...
desplot(block ~ Col + Row | block, flip = TRUE,
        out1 = Row, out1.gpar = list(col = "grey", lty = 1),
        out2 = Col, out2.gpar = list(col = "grey", lty = 1), 
        text = TrtCC, cex = 1, shorten = "no", col=TrtSoil,
        data = fac2rcbd_out$bookRowCol,
        main = "RCBD", 
        show.key = T, key.cex = 0.5)
...
## Three treatment factors RCBD (factorial) ####

fac3rcbd_out <- design.ab(trt = c(n_TrtCC, n_TrtChop, n_TrtSoil), 
                          design = "rcbd",
                          r = n_Reps, 
                          seed = 42)
...
fac3rcbd_out$bookRowCol <- fac3rcbd_out$book %>%
  bind_cols(expand.grid(Row = 1:(n_TrtChop*n_TrtCC),
                        Col = 1:(n_TrtSoil*n_Reps))) %>% 
  mutate(block   = paste0("Block", block),
         TrtPest = paste0("CC", A),
         TrtFert = paste0("Chop", B),
         TrtSoil = paste0("WFPS", C))

# Plot field layout
desplot(TrtSoil ~ Col + Row | block, flip = TRUE,
        out1 = Row, out1.gpar = list(col = "grey", lty = 1),
        out2 = Col, out2.gpar = list(col = "grey", lty = 1), 
        text = TrtCC, cex = 1, shorten = "no", col = TrtSoil, 
