...
PPmeanFUN <- function(dat) {
  
  # aggregate CO2.mg.m3.h data
  mean.co2.mg.m3.h <- aggregate(x = dat$co2.mg.m3.h, 
                        by = list(Treatment = dat$Treatment, Site = dat$site), FUN = function(x) mean(x, na.rm=T))
  
  sd.co2.mg.m3.h <- aggregate(x = dat$co2.mg.m3.h, 
                      by = list(Treatment = dat$Treatment, Site = dat$site), FUN = function(x) sd(x, na.rm=T))
  
  se.co2.mg.m3.h <- aggregate(x = dat$co2.mg.m3.h, 
                      by = list(Treatment = dat$Treatment, Site = dat$site), FUN = function(x) std.error(x, na.rm=T))
  
  med.co2.mg.m3.h <- aggregate(x = dat$co2.mg.m3.h, 
                       by = list(Treatment = dat$Treatment, Site = dat$site), FUN = function(x) median(x, na.rm=T))
  ...
  mean.dat.mg.m3.h <- data.frame(Treatment = mean.co2.mg.m3.h[ ,1], #extract Treatment names from any df
                    ...
                    Site = mean.co2.mg.m3.h[ ,2],
                    mean.co2.mg.m3.h = mean.co2.mg.m3.h[ ,3], # extract mean values from sub df's and compile to main df
                    sd.co2.mg.m3.h = sd.co2.mg.m3.h[ , 3], # CO2 MG
                    se.co2.mg.m3.h = se.co2.mg.m3.h[ , 3],
                    med.co2.mg.m3.h = med.co2.mg.m3.h[ , 3],
                    mean.co2.g.m3.h = mean.co2.g.m3.h[ ,3], # CO2 G
                    sd.co2.g.m3.h = sd.co2.g.m3.h[ , 3],
                    se.co2.g.m3.h = se.co2.g.m3.h[ , 3],
                    med.co2.g.m3.h = med.co2.g.m3.h[ , 3],
                    mean.n2o.mg.m3.h = mean.n2o.mg.m3.h[ ,3], # N2O MG
                    sd.n2o.mg.m3.h = sd.n2o.mg.m3.h[ , 3],
                    se.n2o.mg.m3.h = se.n2o.mg.m3.h[ , 3],
                    med.n2o.mg.m3.h = med.n2o.mg.m3.h[ , 3],
                    mean.ch4.mg.m3.h = mean.ch4.mg.m3.h[ ,3], # CH4 MG
                    sd.ch4.mg.m3.h = sd.ch4.mg.m3.h[ , 3],
                    se.ch4.mg.m3.h = se.ch4.mg.m3.h[ , 3],
                    med.ch4.mg.m3.h = med.ch4.mg.m3.h[ , 3])
